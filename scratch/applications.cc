...
void
EnergyConsumptionUpdate (std:: string filename, double totaloldEnergyConsumption, double totalnewEnergyConsumption)
{
...
main (int argc, char *argv[])
{
  ...
  CommandLine cmd;
  cmd.AddValue("type", "Type of application", app_type);
  cmd.Parse (argc, argv);
  ...
  double simTime=transientDuration + 12;
  ...
  uePositionAlloc->Add (Vector (0, -5, 1.6));
  uePositionAlloc->Add (Vector (250, -5, 1.6));
  ...
  ueNodes.Get (0)->GetObject<MobilityModel> ()->SetPosition (Vector (0, -5, 1.6));
  ueNodes.Get (0)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (0, 0, 0));

  ueNodes.Get (1)->GetObject<MobilityModel> ()->SetPosition (Vector (250, -5, 1.6));
...
//   std::ofstream outFile;
//   std::string energyFileName = "energyfile1.csv";
//   outFile.open (energyFileName,std::ios_base::out | std::ios_base::trunc);
//   outFile << "Time,NetEnergy,DiffEnergy" << std::endl;

//   std::ofstream outFile2;
//   std::string energyFileName2 = "energyfile2.csv";
...
//   deviceEModel.Get(2)->TraceConnectWithoutContext("TotalEnergyConsumption", MakeBoundCallback (&EnergyConsumptionUpdate, "energyfile3.csv"));
  ...
  if(app_type == 0)
  {
        ...
        for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
        {
        VideoStreamClientHelper videoClient (ueIpIface.GetAddress (u), 5000);
        clientApps.Add(videoClient.Install (ueNodes.Get (u)));
        }
        ...
        ApplicationContainer serverApp = videoServer.Install (remoteHost);
  ...
  else if(app_type == 1)
  {
       UdpEchoServerHelper echoServer (9);
        ...
        UdpEchoClientHelper echoClient (InetSocketAddress(remoteHostAddr), 9);
  ...
  else
  {
       PacketSinkHelper sink ("ns3::TcpSocketFactory",
                         InetSocketAddress (remoteHostAddr, 50000));
 
      serverApps = sink.Install (remoteHost);

      Ptr<Socket> localSocket =Socket::CreateSocket (ueNodes.Get (0), TcpSocketFactory::GetTypeId ());
      localSocket->Bind ();

      Ptr<Socket> localSocket1 =Socket::CreateSocket (ueNodes.Get (1), TcpSocketFactory::GetTypeId ());
      localSocket1->Bind ();

      Simulator::ScheduleNow (&StartFlow, localSocket,
                          ueIpIface.GetAddress (0), 50000);

      Simulator::ScheduleNow (&StartFlow, localSocket1,
                          ueIpIface.GetAddress (1), 50000);
      serverApps.Start (Seconds (transientDuration));
