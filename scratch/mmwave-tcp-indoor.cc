...
//	cmd.AddValue("interPacketInterval", "Inter-packet interval [us])", interPacketInterval);
	...
	uemobilityNLOS.Install (ueNodes.Get (2));
	uemobilityNLOS.Install (ueNodes.Get (3));
	...
	uePositionAllocIndoor->Add (Vector (0.0, 40.0, hUT));
	uePositionAllocIndoor->Add (Vector (0.0, 40.0, hUT));
	uemobilityIndoor.SetPositionAllocator(uePositionAllocIndoor);
	uemobilityIndoor.Install (ueNodes.Get (4));
	uemobilityIndoor.Install (ueNodes.Get (5));
