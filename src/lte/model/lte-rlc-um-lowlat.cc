...
LteRlcUmLowLat::DoTransmitPdcpPdu(Ptr<Packet> p)
{
    ...
    if (m_txBufferSize + p->GetSize() <= m_maxTxBufferSize)
    {
        ...
        // m_arrivalRate = (1 - m_forgetFactor) * (p->GetSize () / timeDiff) + m_forgetFactor *
        // m_arrivalRate;
...
LteRlcUmLowLat::DoNotifyTxOpportunity(LteMacSapUser::TxOpportunityParameters txOpParams)
{
    ...
    {
        NS_LOG_DEBUG("LteRlcUmLowLat rnti " << m_rnti << " lcid " << m_lcid << " allocated "
                                            << txOpParams.bytes << " bufsize " << m_txBufferSize);
    }
    ...
    if (!m_reorderingTimer.IsRunning())
    {
        ...
        if (m_vrUh > m_vrUr)
        {
            ...
            m_reorderingTimer = Simulator::Schedule(m_reorderingTimeExpires,
                                                    &LteRlcUmLowLat::ExpireReorderingTimer,
                                                    this);
            m_vrUx = m_vrUh;
...
LteRlcUmLowLat::TriggerReceivePdcpPdu(Ptr<Packet> p)
{
    ...
    {
        NS_LOG_INFO(
            this << " RlcUmLowLat forwards packet to PDCP (either from MmWave or LTE stack)");
