    ...
    static TypeId tid =
        TypeId("ns3::LteUeRrc")
            ...
            .AddAttribute("DataRadioRlcMap",
                          "List of UE RLC for Secondary Connection by LCID.",
                          ObjectMapValue(),
...
LteUeRrc::SwitchLowerLayerProviders(uint16_t cellId)
{
    if (m_isMmWaveCellMap.find(cellId) != m_isMmWaveCellMap.end())
    {
        if (m_isMmWaveCellMap.find(cellId)->second)
        {
            ...
            // NS_LOG_LOGIC("Before switch " << m_cphySapProvider << m_cmacSapProvider <<
            // m_macSapProvider);
            ...
            m_hasReceivedSib2 = false;
            // NS_LOG_LOGIC("After switch " << m_cphySapProvider << m_cmacSapProvider <<
            // m_macSapProvider);
            return true;
        }
        else
...
LteUeRrc::DoReportUeMeasurements(LteUeCphySapUser::UeMeasurementsParameters params)
{
         ...
         ++newMeasIt)
    {
        if (params.m_componentCarrierId != 0)
        {
            ...
            SaveScellUeMeasurements(newMeasIt->m_cellId,
                                    newMeasIt->m_rsrp,
                                    newMeasIt->m_rsrq,
                                    useLayer3Filtering,
    ...
    if (msg.haveSib2)
    {
        ...
        case CONNECTED_REESTABLISHING:
            ...
            if (m_state == CONNECTED_HANDOVER && m_interRatHoCapable && !m_ncRaStarted)
            {
                m_ncRaStarted = true;
                NS_LOG_INFO(
                    "StartNonContentionBasedRandomAccessProcedure after SIB2 received at time "
                    << Simulator::Now());
                m_cmacSapProvider.at(0)->StartNonContentionBasedRandomAccessProcedure(
                    m_rnti,
                    m_rachConfigDedicated.raPreambleIndex,
                    m_rachConfigDedicated.raPrachMaskIndex);
            }
            break;
...
LteUeRrc::DoRecvRrcConnectionReconfiguration(LteRrcSap::RrcConnectionReconfiguration msg)
{
    ...
    case CONNECTED_NORMALLY:
        ...
        {
            NS_LOG_INFO("UE " << m_rnti << " on cellId " << m_cellId
            ...
            else
            {
                ...
                m_asSapUser->NotifySecondaryCellHandoverStarted(oldRnti,
                                                                m_rnti,
                                                                m_cellId,
                                                                msg.radioResourceConfigDedicated);
                msg.radioResourceConfigDedicated.drbToAddModList
                    .clear(); // remove the drb list, since no Data RLC must be setup in this RRC
                // setup SRB1
                ApplyRadioResourceConfigDedicated(msg.radioResourceConfigDedicated);
            }
...
{
    NS_LOG_INFO("Recv RRC Connection Switch on rnti " << m_rnti << " of cell " << m_cellId
                                                      << " m_mmWaveCellId " << m_mmWaveCellId
        ...
        if (m_drbMap.find(*iter) != m_drbMap.end())
        {
            ...
            if (pdcp)
            {
                ...
                if (msg.useMmWaveConnection) // I was on LTE, now I switch to mmWave
                {
                    ...
                    // before resetting the RLC, forward the content of the LTE RLC to the mmWave
                    // RLC check if this rlc is already in the map
                    if (m_drbMap.find(*iter) != m_drbMap.end())
                    {
                    ...
                    lcConfig.priority =
                        m_drbMap.find(*iter)->second->m_logicalChannelConfig.priority;
                    lcConfig.prioritizedBitRateKbps =
                        m_drbMap.find(*iter)->second->m_logicalChannelConfig.prioritizedBitRateKbps;
                    lcConfig.bucketSizeDurationMs =
                        m_drbMap.find(*iter)->second->m_logicalChannelConfig.bucketSizeDurationMs;
                    lcConfig.logicalChannelGroup =
                        m_drbMap.find(*iter)->second->m_logicalChannelConfig.logicalChannelGroup;
                    ...
                    for (uint32_t i = 0; i < m_numberOfComponentCarriers; i++)
                    {
                        m_cmacSapProvider.at(i)->AddLc(lcid, lcConfig, rlc->GetLteMacSapUser());
                    }
                    rlc->Initialize();
                }
                else // I was on mmWave, and I switch to LTE
                {
                    ...
                    {
                        m_mmWaveCmacSapProvider.at(i)->RemoveLc(
                            m_rlcMap.find(*iter)->second->logicalChannelIdentity);
                    }

                    // before resetting, forward the content of the RLC in m_rlcMap to the RLC in
                    // m_drbMap
                    if (m_rlcMap.find(*iter) != m_rlcMap.end())
                    {
                        // get the RLC buffer content and store it in this RLC
                        CopyRlcBuffers(m_rlcMap.find(*iter)->second->m_rlc,
                                       pdcp,
                                       m_rlcMap.find(*iter)->second->logicalChannelIdentity);
                    }
                    ...
                    lcConfig.prioritizedBitRateKbps =
                        m_rlcMap.find(*iter)->second->logicalChannelConfig.prioritizedBitRateKbps;
                    lcConfig.bucketSizeDurationMs =
                        m_rlcMap.find(*iter)->second->logicalChannelConfig.bucketSizeDurationMs;
                    lcConfig.logicalChannelGroup =
                        m_rlcMap.find(*iter)->second->logicalChannelConfig.logicalChannelGroup;

                    // m_mmWaveCmacSapProvider->AddLc
                    // (m_rlcMap.find(*iter)->second->logicalChannelIdentity,
                    //                         lcConfig,
                    //                         rlc->GetLteMacSapUser ());
                    ...
                    for (uint32_t i = 0; i < m_numberOfComponentCarriers; i++)
                    {
                        m_mmWaveCmacSapProvider.at(i)->AddLc(
                            m_rlcMap.find(*iter)->second->logicalChannelIdentity,
                            lcConfig,
                            rlc->GetLteMacSapUser());
                    }
                ...
                {
                    NS_LOG_INFO("LteUeRrc SwitchToMmWave " << m_imsi << m_cellId << m_rnti
                                                           << " at time "
                                                           << Simulator::Now().GetSeconds());
                ...
                {
                    NS_LOG_INFO("LteUeRrc SwitchToLte " << m_imsi << m_cellId << m_rnti
                                                        << " at time "
                                                        << Simulator::Now().GetSeconds());
...
LteUeRrc::CopyRlcBuffers(Ptr<LteRlc> rlc, Ptr<LtePdcp> pdcp, uint16_t lcid)
{
    ...
    if (rlc->GetObject<LteRlcAm>())
    {
        ...
        // m_rlcBufferToBeForwardedSize =
        // drbIt->second->m_rlc->GetObject<LteRlcAm>()->GetTxBufferSize(); m_rlcBufferToBeForwarded
             ...
             ++it)
        {
            ...
            if ((*it)->GetSize() > 3)
            {
                ...
                NS_LOG_DEBUG("UE RRC: txonBuffer SEQ = " << pdcpHeader.GetSequenceNumber()
                                                         << " Size = " << (*it)->GetSize());
            }
            else
            {
        ...
        { // something inside the RLC AM's transmitting buffer
            NS_LOG_DEBUG(
                "UE RRC: ADDING TRANSMITTING SDUS OF RLC AM TO X2FORWARDINGBUFFER... Size = "
            ...
            NS_LOG_DEBUG(this << "UE RRC:  ADDING TXONBUFFER OF RLC AM " << m_rnti
                              << " Size = " << txonBufferSize);
            ...
            m_rlcBufferToBeForwarded.insert(m_rlcBufferToBeForwarded.end(),
                                            txonBuffer.begin(),
                                            txonBuffer.end());
            m_rlcBufferToBeForwardedSize +=
                rlcAm->GetTransmittingRlcSduBufferSize() + txonBufferSize;
                 ...
                 ++it)
            {
                if ((*it) != NULL)
                {
                    ...
                    {
                        NS_LOG_DEBUG("UE RRC: Added previous SDU to forwarding buffer SEQ = "
                                     << pdcpHeader.GetSequenceNumber()
                                     << " Size = " << (*it)->GetSize());
                        m_rlcBufferToBeForwarded.insert(m_rlcBufferToBeForwarded.begin() + i,
                                                        (*it)->Copy());
                        ++i;
                    }
                }
    ...
    {
        NS_LOG_DEBUG(this << " UE RRC: Forwarding m_rlcBufferToBeForwarded to target eNB, lcid = "
                          << lcid);
        ...
        NS_LOG_LOGIC(this << " UE RRC: After forwarding: buffer size = "
                          << m_rlcBufferToBeForwardedSize);
    }
}
...
void
LteUeRrc::DoNotifySecondaryCellHandover(uint16_t oldRnti,
                                        uint16_t newRnti,
                                        ...
                                        LteRrcSap::RadioResourceConfigDedicated rrcd)
{
    ...
    NS_ASSERT_MSG(!m_isSecondaryRRC,
                  "Trying to modify RLCs in the mmWave RRC (they are instances in the LTE RRC)");
    ...
    {
        NS_LOG_INFO(this << " IMSI " << m_imsi << " modifying DRBID "
        ...
        if (dtamIt->is_mc == true) // we need to modify the RLC for MmWave communications
        {
            ...
            if (pdcp)
            {
                ...
                logicalChannelConfig.bucketSizeDurationMs =
                    dtamIt->logicalChannelConfig.bucketSizeDurationMs;
                logicalChannelConfig.logicalChannelGroup =
                    dtamIt->logicalChannelConfig.logicalChannelGroup;
                ...
                {
                    // get the RLC buffer content and store it in this RLC
                    CopyRlcBuffers(m_rlcMap.find(dtamIt->drbIdentity)->second->m_rlc,
                                   drbInfo->m_pdcp,
                                   drbInfo->m_logicalChannelIdentity);
                }
                m_rlcMap[dtamIt->drbIdentity] = rlcInfo;

                NS_LOG_INFO(this << "LteUeRrc Secondary Cell Handover, created new rlc " << m_imsi
                                 << m_mmWaveCellId << m_mmWaveRnti << " at time "
                                 << Simulator::Now().GetSeconds());
                m_switchToMmWaveTrace(m_imsi, m_mmWaveCellId, m_mmWaveRnti);
            }
            else
    ...
    LteRrcSap::NonCriticalExtensionConfiguration nonCec)
{
    ...
    for (std::list<LteRrcSap::SCellToAddMod>::iterator it = nonCec.sCellsToAddModList.begin();
         it != nonCec.sCellsToAddModList.end();
         it++)
    {
        ...
        uint8_t txMode = scell.radioResourceConfigDedicateSCell.physicalConfigDedicatedSCell
                             .antennaInfo.transmissionMode;
        uint8_t srsIndex = scell.radioResourceConfigDedicateSCell.physicalConfigDedicatedSCell
        ...
        LteRrcSap::PdschConfigDedicated pdschConfigDedicated =
            scell.radioResourceConfigDedicateSCell.physicalConfigDedicatedSCell
...
LteUeRrc::ApplyRadioResourceConfigDedicated(LteRrcSap::RadioResourceConfigDedicated rrcd)
{
    ...
    for (dtamIt = rrcd.drbToAddModList.begin(); dtamIt != rrcd.drbToAddModList.end(); ++dtamIt)
    {
        ...
        // std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator drbMapIt = m_drbMap.find
        // (dtamIt->drbIdentity);
        ...
        if (drbMapIt == m_drbMap.end())
        {
            ...
            lcConfigToStore.prioritizedBitRateKbps =
                dtamIt->logicalChannelConfig.prioritizedBitRateKbps;
            lcConfigToStore.bucketSizeDurationMs =
                dtamIt->logicalChannelConfig.bucketSizeDurationMs;
        ...
        else
        {
            ...
            if (dtamIt->is_mc == true) // we need to setup the RLC for MmWave communications
            {
                ...
                if (pdcp)
                {
                    ...
                    NS_LOG_INFO("LteUeRrc SwitchToMmWave and create new rlc "
                                << m_imsi << m_mmWaveCellId << m_mmWaveRnti << " at time "
                                << Simulator::Now().GetSeconds());
                    m_switchToMmWaveTrace(m_imsi, m_mmWaveCellId, m_mmWaveRnti);
                }
                else
            ...
            else
            {
                ...
                // lcConfigToStore.bucketSizeDurationMs =
                // dtamIt->logicalChannelConfig.bucketSizeDurationMs;
                // lcConfigToStore.logicalChannelGroup =
                // dtamIt->logicalChannelConfig.logicalChannelGroup; drbInfo->m_logicalChannelConfig
                // = lcConfigToStore;
                ...
                // dtamIt->logicalChannelConfig.prioritizedBitRateKbps;
                // lcConfig.bucketSizeDurationMs =
                // dtamIt->logicalChannelConfig.bucketSizeDurationMs; lcConfig.logicalChannelGroup =
                // dtamIt->logicalChannelConfig.logicalChannelGroup;
                ...
                // {
                //   // get the RLC buffer content and store it in this RLC
                //   CopyRlcBuffers(m_drbMap.find(dtamIt->drbIdentity)->second->m_rlc,
                //   drbInfo->m_pdcp, drbInfo->m_logicalChannelIdentity);
                // }
...
void
LteUeRrc::SaveUeMeasurements(uint16_t cellId, double rsrp, double rsrq, bool useLayer3Filtering)
{
...
LteUeRrc::MeasurementReportTriggering(uint8_t measId)
{
    ...
    case LteRrcSap::ReportConfigEutra::EVENT_A1: {
        ...
        NS_LOG_LOGIC(this << " event A1: serving cell " << m_cellId << " ms=" << ms
    ...
    case LteRrcSap::ReportConfigEutra::EVENT_A2: {
        ...
        NS_LOG_LOGIC(this << " event A2: serving cell " << m_cellId << " ms=" << ms
                          << " thresh=" << thresh << " entryCond=" << entryCond
                          << " leavingCond=" << leavingCond);
...
LteUeRrc::SendMeasurementReport(uint8_t measId)
{
    ...
    if (cellHasMeasures)
    {
        ...
        NS_LOG_INFO(this << " reporting serving cell "
                            "RSRP "
                         << (uint32_t)measResults.rsrpResult << " (" << servingMeasIt->second.rsrp
                         << " dBm) "
                            "RSRQ "
                         << (uint32_t)measResults.rsrqResult << " (" << servingMeasIt->second.rsrq
                         << " dB)");
        ...
        else
        {
            ...
            if (sCellsMeasIt != m_storedScellMeasValues.end())
            {
                ...
                for (sCellsMeasIt = m_storedScellMeasValues.begin();
                     sCellsMeasIt != m_storedScellMeasValues.end();
                     ++sCellsMeasIt)
                {
                    ...
                    measResultScell.rsrpResult =
                        EutranMeasurementMapping::Dbm2RsrpRange(sCellsMeasIt->second.rsrp);
                    measResultScell.rsrqResult =
                        EutranMeasurementMapping::Db2RsrqRange(sCellsMeasIt->second.rsrq);
...
void
LteUeRrc::SaveScellUeMeasurements(uint16_t sCellId,
                                  double rsrp,
                                  double rsrq,
                                  ...
                                  uint16_t componentCarrierId)
{
    ...
    if (sCellId == m_cellId)
    {
        std::map<uint16_t, MeasValues>::iterator storedMeasIt =
            m_storedScellMeasValues.find(componentCarrierId);
        ...
        if (storedMeasIt != m_storedScellMeasValues.end())
        {
            if (useLayer3Filtering)
            {
                ...
                storedMeasIt->second.rsrp =
                    (1 - m_varMeasConfig.aRsrp) * storedMeasIt->second.rsrp +
                    m_varMeasConfig.aRsrp * rsrp;
        ...
        else
        {
            ...
            std::pair<std::map<uint16_t, MeasValues>::iterator, bool> ret =
                m_storedScellMeasValues.insert(val);
            NS_ASSERT_MSG(ret.second == true, "element already existed");
            storedMeasIt = ret.first;
        }

        NS_LOG_DEBUG(this << " IMSI " << m_imsi << " state " << ToString(m_state)
                          << ", measured cell " << sCellId << ", carrier component Id "
                          << componentCarrierId << ", new RSRP " << rsrp << " stored "
                          << storedMeasIt->second.rsrp << ", new RSRQ " << rsrq << " stored "
                          << storedMeasIt->second.rsrq);
        storedMeasIt->second.timestamp = Simulator::Now();
    }
