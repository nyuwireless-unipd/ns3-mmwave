  ...
  static TypeId tid =
      TypeId ("ns3::MmWavePointToPointEpcHelper")
          ...
          .AddAttribute (
              ...
              MakeTimeAccessor (&MmWavePointToPointEpcHelper::m_s1uLinkDelay), MakeTimeChecker ())
          ...
          .AddAttribute ("S1apLinkDataRate",
                         ...
                         MakeDataRateChecker ())
          .AddAttribute ("S1apLinkDelay", "The delay to be used for the S1-AP link to be created",
                         TimeValue (MilliSeconds (15)),
                         MakeTimeAccessor (&MmWavePointToPointEpcHelper::m_s1apLinkDelay),
                         MakeTimeChecker ())
          .AddAttribute ("S1apLinkMtu", "The MTU of the next S1-AP link to be created",
                         UintegerValue (10000),
          ...
          .AddAttribute ("X2LinkDataRate",
                         ...
                         MakeDataRateChecker ())
          .AddAttribute ("X2LinkDelay", "The delay to be used for the next X2 link to be created",
                         TimeValue (Seconds (0.001)),
...
MmWavePointToPointEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
  ...
  Ptr<EpcEnbApplication> enbApp = CreateObject<EpcEnbApplication> (
      enbLteSocket, enbLteSocket6, enbS1uSocket, enbAddress, sgwAddress, cellId);
  ...
  uint16_t mmeId = 1;
  Ptr<EpcS1apEnb> s1apEnb = CreateObject<EpcS1apEnb> (enbS1apSocket, mme_enbAddress, mmeAddress,
                                                      cellId, mmeId); // only one mme!
...
uint8_t
MmWavePointToPointEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, uint64_t imsi,
...
uint8_t
MmWavePointToPointEpcHelper::ActivateEpsBearer (Ptr<NetDevice> ueDevice, Ptr<EpcUeNas> ueNas,
                                                uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer)
{
  NS_LOG_FUNCTION (this << ueDevice << imsi);
