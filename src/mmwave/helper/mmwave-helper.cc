...
MmWaveHelper::InstallSingleMcUeDevice(Ptr<Node> n)
{
         ...
         ++it)
    {
        ...
        dlPhy->AddInterferenceCtrlChunkProcessor (pInterf);   // for RSRQ evaluation of UE
        Measurements
         ...
         ++it)
    {
        ...
        // ccPhy->GetDlSpectrumPhy ()->SetLtePhyRxPssCallback (MakeCallback (&LteUePhy::ReceivePss,
        // ccPhy)); ccPhy->GetDlSpectrumPhy ()->SetLtePhyDlHarqFeedbackCallback (MakeCallback
        // (&LteUePhy::ReceiveLteDlHarqFeedback, ccPhy)); this is done before
    }
...
MmWaveHelper::InstallSingleInterRatHoCapableUeDevice(Ptr<Node> n)
{
    ...
    (&MmWaveUePhy::ReceiveLteDlHarqFeedback, mmWavePhy));
    }
    ...
    Ptr<MobilityModel> mm = n->GetObject<MobilityModel> ();
    NS_ASSERT_MSG (mm, "MobilityModel needs to be set on node before calling
    MmWaveHelper::InstallUeDevice ()"); mmWaveUlPhy->SetMobility(mm); mmWaveDlPhy->SetMobility(mm);
    ...
    mmWaveDlPhy->SetPhyRxDataEndOkCallback (MakeCallback (&MmWaveUePhy::PhyDataPacketReceived,
    mmWavePhy)); mmWaveDlPhy->SetPhyRxCtrlEndOkCallback (MakeCallback
    (&MmWaveUePhy::ReceiveControlMessageList, mmWavePhy));
    ...
    {
            Ptr<MmWaveLteUeRrcProtocolReal> rrcProtocol = CreateObject<MmWaveLteUeRrcProtocolReal>
    (); rrcProtocol->SetUeRrc (rrc); rrc->AggregateObject (rrcProtocol);
            rrcProtocol->SetLteUeRrcSapProvider (rrc->GetLteUeRrcSapProvider ());
            rrc->SetLteUeRrcSapUser (rrcProtocol->GetLteUeRrcSapUser ());
    }
    ...
    lteDlPhy->SetLtePhyRxCtrlEndOkCallback (MakeCallback (&LteUePhy::ReceiveLteControlMessageList,
    ltePhy)); lteDlPhy->SetLtePhyRxPssCallback (MakeCallback (&LteUePhy::ReceivePss, ltePhy));
    lteDlPhy->SetLtePhyDlHarqFeedbackCallback (MakeCallback (&LteUePhy::ReceiveLteDlHarqFeedback,
    ltePhy)); lteNas->SetForwardUpCallback (MakeCallback (&McUeNetDevice::Receive, device));
    ...
    for (auto it = ueCcMap.begin(); it != ueCcMap.end(); ++it)
    {
        ...
        bool ccmTest =
            ccmUe->SetComponentCarrierMacSapProviders(it->first,
                                                      ccUe->GetMac()->GetUeMacSapProvider());
...
MmWaveHelper::InstallSingleEnbDevice(Ptr<Node> n)
{
    ...
    for (auto it = ccMap.begin(); it != ccMap.end(); ++it)
    {
        ...
        Ptr<MobilityModel> mm = n->GetObject<MobilityModel>();
        NS_ASSERT_MSG(mm,
                      "MobilityModel needs to be set on node before calling "
    ...
    for (auto it = ccMap.begin(); it != ccMap.end(); ++it)
    {
        ...
        // FFR SAP
        /* not used in mmwave
  it->second->GetFfMacScheduler ()->SetLteFfrSapProvider (it->second->GetFfrAlgorithm
  ()->GetLteFfrSapProvider ()); it->second->GetFfrAlgorithm ()->SetLteFfrSapUser
  (it->second->GetFfMacScheduler ()->GetLteFfrSapUser ()); rrc->SetLteFfrRrcSapProvider
  (it->second->GetFfrAlgorithm ()->GetLteFfrRrcSapProvider (), it->first);
  ...
  //FFR SAP END*/
    ...
    /*to do for each cc
          ...
          *dlPhy->SetPhyRxDataEndOkCallback (MakeCallback (&MmWaveEnbPhy::PhyDataPacketReceived,
    phy)); *dlPhy->SetPhyRxCtrlEndOkCallback (MakeCallback (&MmWaveEnbPhy::PhyCtrlMessagesReceived,
    phy));
    * dlPhy->SetPhyUlHarqFeedbackCallback (MakeCallback (&MmWaveEnbPhy::ReceiveUlHarqFeedback,
    phy));
  */
    for (auto it = ccMap.begin(); it != ccMap.end(); ++it)
    {
...
// for MC devices
void
MmWaveHelper::AttachToClosestEnb(NetDeviceContainer ueDevices,
...
void
MmWaveHelper::AttachMcToClosestEnb(Ptr<NetDevice> ueDevice,
                                   NetDeviceContainer mmWaveEnbDevices,
                                   NetDeviceContainer lteEnbDevices)
{
    NS_LOG_FUNCTION(this);
         ...
         ++i)
    {
        ...
        for (auto itEnb = mmWaveEnbCcMap.begin(); itEnb != mmWaveEnbCcMap.end(); ++itEnb)
        {
            ...
            // closestMmWave->GetMac ()->AssociateUeMAC (mcDevice->GetImsi ()); //TODO this does not
            // do anything
...
void
MmWaveHelper::AttachIrToClosestEnb(Ptr<NetDevice> ueDevice,
                                   NetDeviceContainer mmWaveEnbDevices,
                                   NetDeviceContainer lteEnbDevices)
{
    NS_LOG_FUNCTION(this);
         ...
         ++i)
    {
        ...
        // closestMmWave->GetMac ()->AssociateUeMAC (mcDevice->GetImsi ()); //TODO this does not do
        // anything
        NS_LOG_INFO("mmWaveCellId " << mmWaveCellId);
...
void
MmWaveHelper::AttachToEnbWithIndex(Ptr<NetDevice> ueDevice,
...
class MmWaveDrbActivator : public SimpleRefCount<MmWaveDrbActivator>
{
  public:
    ...
    static void ActivateCallback(Ptr<MmWaveDrbActivator> a,
                                 std::string context,
                                 uint64_t imsi,
                                 uint16_t cellId,
                                 uint16_t rnti);
...
void
MmWaveDrbActivator::ActivateCallback(Ptr<MmWaveDrbActivator> a,
                                     std::string context,
                                     uint64_t imsi,
                                     uint16_t cellId,
                                     uint16_t rnti)
