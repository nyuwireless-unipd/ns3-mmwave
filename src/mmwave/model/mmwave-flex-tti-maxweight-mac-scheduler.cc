...
MmWaveFlexTtiMaxWeightMacScheduler::DoSchedDlRlcBufferReq (const struct MmWaveMacSchedSapProvider::SchedDlRlcBufferReqParameters& params)
{
  ...
  std::list<MmWaveMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator itRlcBuf = m_rlcBufferReq.begin ();
  bool newLc = true;
	uint8_t lcid = params.m_logicalChannelIdentity;
	while (itRlcBuf != m_rlcBufferReq.end ())
	{
		// remove old entries of this UE-LC
		if ((itRlcBuf->m_rnti == params.m_rnti)&&(itRlcBuf->m_logicalChannelIdentity == lcid))
		{
			...
			itRlcBuf = m_rlcBufferReq.erase (itRlcBuf);
			newLc = false;
		}
		else
		{
  ...
  m_rlcBufferReq.insert (itRlcBuf, params);
...
/*
	...
	if (m_algorithm == EDF) 		// Earliest Deadline First algorithm
	{
		...
		while ((dlSymAvail + ulSymAvail) > 0)
		{
			...
			while (!flowFound && flowIt != m_flowHeap.end () && (dlSymAvail + ulSymAvail) > 0)		// find flow with CQI in range (not in outage)
			{
				...
				if (!flow->m_isUplink && dlSymAvail > 0)
				{
					...
					if (cqi != 0)
					{
						...
						if (numSymReq <= (unsigned)dlSymAvail)	// sufficient symbols to TX whole RLC PDU at this MCS
						{
						...
						else	// insufficient symbols, allocate remaining symbols (must segment RLC PDU)
						{
							...
							uint32_t tbSizeBits = m_amc->GetTbSizeFromMcsSymbols (ueInfo->m_dlMcs, ueInfo->m_dlSymbols + dlSymAvail);
							pduSize = ceil(tbSizeBits/8.0) - ueInfo->m_dlTbSize;
							flow->m_txPacketSizes.front () -= pduSize;		// subtract from HOL packet
							ueInfo->m_dlSymbols += dlSymAvail;
							ueInfo->m_dlTbSize += pduSize;
							dlSymAvail = 0;
						}
				...
				else if (flow->m_isUplink && ulSymAvail > 0)
				{
					...
					if (cqi != 0)
					{
						...
						if (numSymReq <= (unsigned)ulSymAvail)	// sufficient symbols to TX whole RLC PDU at this MCS
						{
							flow->m_txPacketSizes.pop_front ();
							double oldHolDelay = flow->m_txQueueHolDelay;
							ueInfo->m_ulSymbols += numSymReq;		// add to total symbols/bits for UE
							ueInfo->m_ulTbSize += pduSize;
							ulSymAvail -= numSymReq;
							flow->m_txPacketDelays.pop_front ();
							if (flow->m_txPacketDelays.size () > 0)
							{
								flow->m_txQueueHolDelay = flow->m_txPacketDelays.front ();
						...
						{
							// get maximum TB size from MCS and available symbols
							uint32_t tbSizeBits = m_amc->GetTbSizeFromMcsSymbols (ueInfo->m_ulMcs, ueInfo->m_ulSymbols + ulSymAvail);
							pduSize = ceil(tbSizeBits/8.0) - ueInfo->m_ulTbSize;
							flow->m_txPacketSizes.front () -= pduSize;		// subtract from HOL packet
							ueInfo->m_ulSymbols += ulSymAvail;
							ueInfo->m_ulTbSize += pduSize;
							ulSymAvail = 0;
						}
						uint32_t sduSize = pduSize - (m_rlcHdrSize + m_subHdrSize);
						...
						flow->m_totalBufSize -= sduSize;
						flow->m_schedPacketSizes.push_front (sduSize);
						if (flow->m_schedPacketSizes.size () >
						(m_phyMacConfig->GetL1L2CtrlLatency () + m_phyMacConfig->GetUlSchedDelay ()))
						{
							flow->m_totalSchedSize -= flow->m_schedPacketSizes.back ();
							flow->m_schedPacketSizes.pop_back ();
						}
	...
	for (itUeAllocMap = ueAllocMap.begin (); itUeAllocMap != ueAllocMap.end(); itUeAllocMap++)
	{
		...
		if (ueInfo->m_dlSymbols > 0)
		{
			...
			NS_ASSERT (dlSymIdx < m_phyMacConfig->GetSymbolsPerSubframe () - reservedSym);
			dlSymIdx += ueInfo->m_dlSymbols;
			...
			SlotAllocInfo slotInfo (dlSlotIdx++, SlotAllocInfo::DL, SlotAllocInfo::CTRL_DATA, SlotAllocInfo::DIGITAL, ueInfo->m_rnti);
			slotInfo.m_dci = dci;
		...
		if (ueInfo->m_ulSymbols > 0)
		{
			...
			ulSymIdx -= ueInfo->m_ulSymbols;			// assign indices in reverse/decrementing order
			...
			NS_ASSERT (dci.m_harqProcess < m_phyMacConfig->GetNumHarqProcess ());
			SlotAllocInfo slotInfo (ulSlotIdx++, SlotAllocInfo::UL, SlotAllocInfo::CTRL_DATA, SlotAllocInfo::DIGITAL, ueInfo->m_rnti);
			slotInfo.m_dci = dci;
