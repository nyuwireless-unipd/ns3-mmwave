...
(LEDBAT), TCP Low Priority (TCP-LP) and TCP BBR also supported. The model also supports
...
TCP-BBR is a congestion based congestion control algorithm. It seeks to operate at
optimal point where sender experiences maximum delivery rate with minimum RTT.
To do so, it creates a network model comprises of maximum delivery rate with minimum RTT
observed so far. With the help of this model, BBR estimates BDP (max. bandwidth * minimum RTT)
and controls the speed of sending data and the maximum amount of data can be inflight.
...
Following is the highlevel BBR algorithm:
...
* SendQuantum is not yet supported.
* BBR paper discusses traffic policing which can be a possible enhancement.

More information (BBR Internet Draft): https://tools.ietf.org/html/draft-cardwell-iccrg-bbr-congestion-control-00
